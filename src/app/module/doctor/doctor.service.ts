      ...
      cloudinary.uploader
        .upload_stream(
          ...
          async (error, result) => {
            ...
            if (!result) {
              return reject(new AppError(httpStatus.INTERNAL_SERVER_ERROR, "No result from Cloudinary upload"));
            }
            resolve(result);
          },
        )
...
const verifyDoctorEmail = async (payload: IVerifyDoctorEmailPayload) => {
  ...
  if (!existingUser) {
    throw new AppError(httpStatus.NOT_FOUND, "Doctor Appication Not Found. Please Apply Again");
  }
